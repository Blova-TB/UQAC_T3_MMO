...
use bytes::Bytes;
use mathtools::Vec2;
use shared::models::{HandoffComplete, HandoffDrop, HandoffRequest, PlayerJoinUpdate, PositionUpdate, ServerBinaryPacket, ServerHandShake, ShutdownServer, SpawnServer, Subscribe, Unsubscribe};
...
pub struct SpatialService {
    ...
    pub ghost_client: AHashMap<ClientId,Vec<ShardId>>, // client_id -> shard_id : is replicate in ghost on this serv ? (if exist alors le shard est pret à recevoir l'autorité)
...
impl SpatialService {
    ...
    pub fn subdivide_node(&mut self, shard_id: ShardId) ->Option<Vec<(PeerType, Bytes)>>{
        ...
        for &(player_id, shard_id) in &old_sub {
            let unsub = Unsubscribe {
                client_id: player_id.into(),
                topic_id: shard_id.into(),
            };
            outgoing_packets.push((PeerType::Broker, unsub.to_bytes()));
        }
