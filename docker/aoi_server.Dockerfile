...
RUN cargo chef cook --release --recipe-path recipe.json -p aoi_server

COPY . .
# Compilation du binaire final
RUN cargo build --release -p aoi_server
...
COPY --from=builder /app/target/release/aoi_server /usr/local/bin/aoi_server
